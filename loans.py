...
COLLECTION DAY:
  Weekly  : NEXT WEEK's collection day after disbursement (current week SKIPPED)
            Example: Disb Monday → Next Wednesday (if collection day is Wednesday)
            Then every +7 days: Next Wed, Following Wed, etc.
  Monthly : center's collection_day of month, starting next month
  
WEEKLY COLLECTION RULE:
  • Disbursement week is ALWAYS SKIPPED (no collection Mon-Sun of disb week)
  • First EMI = Collection day in the WEEK AFTER disbursement
  • Subsequent EMIs = First EMI + 7, +14, +21 days (weekly intervals)
  
  Example:
    Center: Collection day = Wednesday
    Disbursement: Monday March 10, 2026 (Week 11)
    
    This week's Wednesday = March 12 → SKIP (same week as disbursement)
    Next week's Wednesday = March 19 → First EMI due ✅
    Following Wednesday = March 26 → Second EMI due
    Next Wednesday = April 2 → Third EMI due
─────────────────────────────────────────────────────────────────────────────
"""
...
def get_next_week_collection_day(disb_date, day_value):
    """
    Get collection day in the WEEK AFTER disbursement week.
    
    Rule: Current week (Mon-Sun) is SKIPPED. First EMI is NEXT week's collection day.
    
    Args:
        disb_date: Disbursement date
        day_value: Can be EITHER:
                   - Number: 0=Monday, 1=Tuesday, 2=Wednesday, 3=Thursday, 
                            4=Friday, 5=Saturday, 6=Sunday
                   - String: 'Monday', 'Tuesday', 'Wednesday', etc.
    
    Returns:
        Date of first collection (always in next week)
    
    Raises:
        ValueError: If day_value is NULL, empty, or invalid
    
    Example:
        Disb: Monday March 10 (Week 11)
        day_value: 1 (Tuesday) OR 'Tuesday'
        
        Step 1: Find end of Week 11 = Sunday March 16
        Step 2: Find Tuesday AFTER Sunday = Tuesday March 18 (Week 12)
        Result: First EMI = March 18 (next week's Tuesday)
    """
    ...
    # CRITICAL: Validate day_value is not NULL/empty
    if day_value is None or str(day_value).strip() == '':
        raise ValueError(
            "❌ ERROR: Center's weekly_collection_day is NULL or empty! "
            "Please run this SQL to fix:\n"
            "UPDATE Center SET weekly_collection_day = 'Tuesday' WHERE id = [CENTER_ID]\n"
            "Valid values: Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday"
        )
    
    # Convert day_value to weekday number (0-6)
    if isinstance(day_value, int):
        # Already a number (0-6) - validate range
        if day_value < 0 or day_value > 6:
            raise ValueError(
                f"❌ ERROR: Invalid weekday number '{day_value}'! "
                f"Must be 0-6 (0=Monday, 6=Sunday)"
            )
        target_weekday = day_value
        
    elif isinstance(day_value, str):
        # String day name - convert to number
        day_upper = day_value.strip().upper()
        
        if day_upper not in day_name_map:
            raise ValueError(
                f"❌ ERROR: Invalid collection day '{day_value}'! "
                f"Valid values: Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday"
            )
        
        target_weekday = day_name_map[day_upper]
        
    else:
        # Unknown type - try to convert to int
        try:
            target_weekday = int(day_value)
            if target_weekday < 0 or target_weekday > 6:
                raise ValueError()
        except:
            raise ValueError(
                f"❌ ERROR: Invalid collection day type '{type(day_value).__name__}'! "
                f"Must be string (e.g., 'Tuesday') or number (0-6)"
            )
    
    # Step 1: Find Sunday of current week (end of disbursement week)
    # weekday(): Mon=0, Tue=1, ..., Sun=6
    days_until_sunday = 6 - disb_date.weekday()
    week_end = disb_date + timedelta(days=days_until_sunday)
    
    # Step 2: Find target weekday AFTER week_end
    days_ahead = target_weekday - week_end.weekday()
    if days_ahead <= 0:
        days_ahead += 7
    
    first_collection = week_end + timedelta(days=days_ahead)
...
def disburse_loan(app_id):
    ...
    try:
        ...
        if is_weekly:
            if isinstance(weekly_coll_day, int):
                collection_info = weekdays[weekly_coll_day]
            else:
                collection_info = weekly_coll_day or 'Monday'
        else:
...
def confirm_disburse(app_id):
    ...
    try:
        ...
        # ═══════════════════════════════════════════════════════════════════
        # CALCULATE FIRST EMI DUE DATE (WEEKLY: SKIP CURRENT WEEK)
        # ═══════════════════════════════════════════════════════════════════
        if is_weekly:
            # Get collection day in NEXT WEEK (current week skipped)
            first_emi_due = get_next_week_collection_day(disb_date, weekly_coll_day)
            
            # Log for verification
            disb_week = disb_date.isocalendar()[1]
            first_week = first_emi_due.isocalendar()[1]
            days_to_first = (first_emi_due - disb_date).days
            
            logger.info(
                f"Weekly loan disbursed: {disb_date.strftime('%A %d-%b-%Y')} (Week {disb_week}) | "
                f"Collection day: {weekly_coll_day} | "
                f"First EMI: {first_emi_due.strftime('%A %d-%b-%Y')} (Week {first_week}) | "
                f"Days to first EMI: {days_to_first}"
            )
        else:
            # Monthly: next month, on collection_day
            target_day = int(collection_day) if collection_day else 15
            first_emi_due = add_months(disb_date, 1, target_day)
        ...
        for emi_number in range(1, tenure + 1):
            ...
            if is_weekly:
                # ═════════════════════════════════════════════════════════════
                # WEEKLY LOAN: First EMI = next week's collection day
                #              Then add +7, +14, +21 days for subsequent EMIs
                # ═════════════════════════════════════════════════════════════
                # EMI #1: first_emi_due (e.g., next week's Wednesday)
                # EMI #2: first_emi_due + 7 days (following Wednesday)
                # EMI #3: first_emi_due + 14 days (Wednesday after that)
                
                due_date = first_emi_due + timedelta(days=(emi_number - 1) * 7)
            else:
