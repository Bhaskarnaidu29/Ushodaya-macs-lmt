...
try:
    from advance import get_advance_balance, get_advance_balance_cursor
...
def _apply_advance_manually(cursor, member_code, loanid, loanrecid, amount_needed, user_id):
    ...
    Uses the EXISTING cursor to avoid opening a second DB connection
    mid-transaction (which causes deadlocks / hanging with pyodbc pooling).
    ...
    try:
        # Use cursor-based balance — reuses the open transaction connection
        balance = get_advance_balance_cursor(cursor, member_code, loanid)
        ...
        # Calculate new balance using cursor (includes the Debit row just inserted above)
        new_balance = get_advance_balance_cursor(cursor, member_code, loanid)
...
def recovery_posting():
    ...
    try:
        ...
        if selected_center:
            ...
            # Get advance balances — use cursor-based helper to reuse the open connection
            if ADVANCE_ENABLED and members:
                for m in members:
                    ...
                    try:
                        ...
                        balance = get_advance_balance_cursor(c, member_code, loanid)
