...
def get_advance_balance(member_code, loanid):
    """
    Calculate available advance balance — opens its own connection.
    Use ONLY when no transaction is already in progress.
    For use inside an active transaction, call get_advance_balance_cursor() instead.
    """
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        balance = _calc_balance_on_cursor(cursor, member_code, loanid)
        cursor.close()
    finally:
        conn.close()
    return balance
...
def get_advance_balance_cursor(cursor, member_code, loanid):
    """
    Calculate available advance balance using an EXISTING cursor/connection.
    Call this when already inside an open transaction to avoid opening a
    second connection (which causes deadlocks / hanging with pyodbc pooling).
    """
    return _calc_balance_on_cursor(cursor, member_code, loanid)


def _calc_balance_on_cursor(cursor, member_code, loanid):
    """Shared balance calculation — works on any cursor."""
    cursor.execute("""
        SELECT
            ISNULL(SUM(CASE WHEN creditdebit = 'Credit' THEN amount ELSE 0 END), 0) -
            ISNULL(SUM(CASE WHEN creditdebit = 'Debit'  THEN amount ELSE 0 END), 0)
        FROM AdvanceRecovery
        WHERE member_code = ? AND loanid = ?
    """, (member_code, loanid))
...
def show_add_advance_form():
    ...
    return render_template("advance_add.html", now=now)
...
def add_advance():
    ...
    try:
        ...
        # Get new balance AFTER closing the insert connection
...
def apply_manual():
    ...
    try:
        ...
        # Get new balance AFTER closing the transaction connection
        new_balance = get_advance_balance(member_code, loanid)
